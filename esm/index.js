...
export function applyMagic(target, ctx = false) {
    if (typeof target === "function") {
        if (ctx === true) {
            return proxify(target);
        }
        const PseudoClass = function PseudoClass(...args) {
            ...
            if (typeof new.target === "undefined") { // function call
                ...
                if (invoke && !warnedInvokeDeprecation) {
                    ...
                    console.warn("applyMagic: using __invoke without 'static' modifier is deprecated");
                }
                checkType(target, invoke, "__invoke");
...
function checkType(ctor, fn, name, argLength = void 0) {
    if (fn !== undefined) {
        if (typeof fn != "function") {
            throw new TypeError(`${ctor.name}.${name} must be a function`);
        }
        else if (argLength !== undefined && fn.length !== argLength) {
            throw new SyntaxError(`${ctor.name}.${name} must have ` +
                `${argLength} parameter${argLength === 1 ? "" : "s"}`);
        }
    }
}
function setProp(target, prop, value, writable = false) {
